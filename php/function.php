...
function getMessages(){
    ...
    $query = "SELECT messages.message,messages.created_at,users.full_name,users.id FROM messages JOIN users ON messages.user_id=users.id ORDER BY messages.id ASC";
    $run = mysqli_query($con,$query);
...
function getTypingStatus($data){
    $con = $GLOBALS['db'];
$query = "SELECT users.full_name FROM typing_status JOIN users ON users.id=typing_status.user_id WHERE typing_status.status=1 AND typing_status.user_id!=".$data['user_id'];
$run=mysqli_query($con,$query);
